...
--○ 오라클 사용자 계정 생성
CREATE USER lsr IDENTIFIED BY java006$
DEFAULT TABLESPACE TBS_EDUA;
--==>> lsr 라는 사용자를 만들겠다. (생성하겠다.)
--     패스워드는 java006$로 하겠다.
--     이 계정을 통해 생성하는 오라클 객체는(세그먼트들은)
--     기본적으로(default) TBS_EDUA라는 테이블스페이스에 생성할 수 있도록 설정하겠다.
--==>> User LSR이(가) 생성되었습니다.


--※ 생성된 오라클 사용자 계정(lsr)을 통해
--   접속을 시도해 보았으나 접속 불가.
--   → 『CREATE SESSION』 권한이 없기 때문에..

--○ 생성된 오라클 사용자 계정(lsr)에
--   서버 접속이 가능할 수 있도록 CREATE SESSION 권한 부여 → SYS가
GRANT CREATE SESSION TO lsr;
--==>> Grant을(를) 성공했습니다.


--○ 생성된 오라클 사용자 계정(LSR)의
...
/*
          :
LSR	CREATE SESSION	NO
          :
LSR이라는 이름 가진 GRANTEE가 CREATE SESSION 권한을 가지고 있음
*/


--○ 생성된 오라클 사용자 계정(LSR)에
--   테이블 생성이 가능할 수 있도록 CREATE TABLE 권한 부여
GRANT CREATE TABLE TO LSR;
--==>> Grant을(를) 성공했습니다.
