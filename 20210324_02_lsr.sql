...
--NUMBER(10)은 숫자 길이 설정 10자리
--VARCHAR2(30)은 문자 길이 설정
--==>> 에러 발생
/*
...
--> 현재 lsr 계정은 CREATE SESSION 권한만 갖고 있으며
--  테이블을 생성할 수 있는 권한은 갖고 있지 않은 상태이다.
--  그러므로 관리자로부터 테이블 생성 권한을 부여받아야 한다.

--○ SYS로부터 CREAT TABLE 권한 받은 후
...
--  lsr 사용자 계정의 기본 테이블 스페이스(DEFAULT TABLESPACE → TBS_EDUA)에 대한
--  할당량을 부여받지 못한 상태.
