...
SALGRADE	        TABLE	→ 급여(SAL) 등급 정보 테이블
TBL_EXAMPLE1	    TABLE	
TBL_EXAMPLE2	    TABLE	
*/
...
/*
1. 숫자형 NUMBER       → -10의 38슬-1 ~ 10의 38승
          NUMBER(3)    → -999 ~ 999
          NUMBER(4)    → -9999 ~ 9999
...
2. 문자형 CHAR         - 고정형 크기
          ...
          CHAR(10) ← 'OH잠깐박민지'   10Byte를 초과하므로 입력 불가
          
          VARCHAR2     - 가변형 크기
          VARCHAR2(10) ← '강의실'          6Byte
          VARCHAR2(10) ← '잠깬박민지'     10Byte
          VARCHAR2(10) ← 'OH잠깐박민지'   10Byte를 초과하므로 입력 불가
...
--==>> Table EMPCOPY4이(가) 생성되었습니다.

SELECT *
FROM EMPCOPY4;

INSERT INTO EMPCOPY4 VALUES(... ..., ..., ..., ..., ...);
...
SELECT ENAME "사원명", EMPNO "사원번호", SAL "급여", COMM "커미션", SAL*12 "연봉"
FROM TBL_EMP;
...
--  NVL() 나 NVL2() 와 비교하여 모~~든 경우의 수를 고려할 수 있는 특징을 갖고 있다.
