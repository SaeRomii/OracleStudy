...
--==>>
/*
BIN$NSphFd3HThKHgp4Eo7rrcQ==$0	TABLE	
BIN$uVd0ByetR5KRzey5Q3V12w==$0	TABLE	
BONUS	                        TABLE	
DEPT	                        TABLE	
EMP	                            TABLE	
EMPCOPY1	                    TABLE	
...
SELECT NAME, SSN
FROM TBL_INSA
WHERE SUBSTR(SSN, 1, 2) >= 80 AND SUBSTR(SSN, 1, 2) <=89;
--==>>
/*
이순신	801007-1544236
한석봉	811112-1566789
...
SELECT *
FROM TBL_INSA
WHERE CITY = '서울' AND (SUBSTR(SSN, 1, 2) >=70 AND SUBSTR(SSN, 1, 2) <=79);
--==>>
/*
...
SELECT *
FROM TBL_INSA
WHERE CITY = '서울' AND (SUBSTR(SSN, 1, 2) >= 70 AND SUBSTR(SSN, 1, 2) <=79) AND SUBSTR(SSN, 8, 1) IN ('1', '3');
--==>>
/*
1001    	홍길동	771212-1022432	1998-10-11 00:00:00	서울	011-2356-4528	기획부	부장	2610000	200000
1019	    박문수	780710-1985632	1999-12-10 00:00:00	서울	017-4747-4848	인사부	과장	2300000	165000
...
FROM TBL_INSA
WHERE SUBSTR(IBSADATE, 1, 2) = '20';
--==>>
/*
이순신	경기	2000-11-29 00:00:00
김정훈	전북	2000-10-01 00:00:00
한석봉	서울	2004-08-13 00:00:00
이기자	인천	2002-02-11 00:00:00
김영년	서울	2002-04-30 00:00:00
나윤균	경기	2003-10-10 00:00:00
유관순	서울	2000-07-07 00:00:00
황진이	인천	2002-02-15 00:00:00
이상헌	경기	2001-11-29 00:00:00
엄용수	인천	2000-08-28 00:00:00
이성길	전북	2004-08-08 00:00:00
유영희	전남	2003-10-10 00:00:00
홍길남	경기	2001-09-07 00:00:00
이영숙	전남	2003-02-25 00:00:00
우재옥	서울	2000-10-01 00:00:00
김숙남	경기	2002-08-28 00:00:00
김영길	서울	2000-10-18 00:00:00
이남신	제주	2001-09-07 00:00:00
김말숙	서울	2000-09-08 00:00:00
지재환	서울	2001-01-21 00:00:00
심심해	전북	2000-05-05 00:00:00
이정석	경기	2005-09-26 00:00:00
정영희	인천	2002-05-16 00:00:00
이재영	서울	2003-08-10 00:00:00
고순정	경기	2003-12-28 00:00:00
박세열	경북	2000-09-10 00:00:00
문길수	충남	    2001-12-10 00:00:00
채정희	경기	2003-10-17 00:00:00
양미옥	서울	2003-09-24 00:00:00
지수환	서울	2004-01-21 00:00:00
홍원신	전북	2003-03-16 00:00:00
산마루	서울	2001-07-15 00:00:00
이기상	전남	2001-06-07 00:00:00
이미성	인천	2000-04-07 00:00:00
이미인	경기	2003-06-07 00:00:00
권영미	서울	2000-06-04 00:00:00
권옥경	경기	2000-10-10 00:00:00
정한나	서울	2004-06-07 00:00:00
전용재	인천	2004-08-13 00:00:00
김신제	인천	2003-08-08 00:00:00
임수봉	서울	2001-10-10 00:00:00
김신애	서울	2001-10-10 00:00:00
*/
...
박민지	960907-2234567	기획부	5000000
...
--32. 전화번호가 있으면 '-'을 제거하여 조회하고, 
    없으면 '전화번호없음'으로 조회.
SELECT NAME "이름", NVL2(TEL, REPLACE(TEL, '-', ''), '전화번호없음') "전화번호"
FROM TBL_INSA;
--==>>
/*
이름	전화번호
...
--33. TBL_INSA 테이블에서 BASICPAY + SUDANG 이 
    ...
    200만원 이상인 직원들의 수 조회.
SELECT T.급여, COUNT(T.급여) "직원수"
FROM
(   
    SELECT CASE WHEN (BASICPAY+SUDANG) >= 2000000 THEN '200만원 이상'
                WHEN (BASICPAY+SUDANG) >= 1000000 THEN '100만원이상 200만원 미만'
...
SELECT '19' || SUBSTR(SSN, 1, 2) "년도", COUNT(*) "직원수"
FROM TBL_INSA
...
SELECT BUSEO, COUNT(*)
FROM TBL_INSA
WHERE DECODE(BUSEO, '개발부', 0, '영업부', 0, '총무부', 0, 1) = 0
GROUP BY BUSEO;
--==>>
/*
총무부	7
...
FROM
(
    SELECT DECODE(SUBSTR(SSN, 8, 1),  '1', '남자', '2', '여자', '성별확인불가') "성별"
    FROM TBL_INSA
    WHERE BUSEO = '영업부'
)T
GROUP BY T.성별;
...
--==>>
/*
여자	1659045.16129032258064516129032258064516
...
FROM
(
    SELECT EXTRACT(YEAR FROM IBSADATE) || '년도' "입사년도"
        , CASE WHEN SUBSTR(SSN, 8, 1) = '1' THEN '남자'
               WHEN SUBSTR(SSN, 8, 1) = '2' THEN '여자'
               ELSE '알수없음'
          END "성별"
...
SELECT T.부서, COUNT(*) "인원수", SUM(T.급여) "급여합"
FROM
(
    SELECT BUSEO "부서"
        , CASE WHEN SUBSTR(SSN, 8, 1) = '1' THEN '남자'
...
SELECT NAME "이름"
     , CASE WHEN SUBSTR(SSN, 8, 1) IN('1', '2') 
            THEN SUBSTR(SSN, 1, 2)+1900 || '-' || SUBSTR(SSN, 3, 2) || '-' || SUBSTR(SSN, 5, 2)
            END "생년월일"
...
박민지	1996-09-07	￦5,000,000	500000
*/
...
SELECT NAME "이름", SSN "주민번호"
     , EXTRACT(YEAR FROM IBSADATE) || '년-' || EXTRACT(MONTH FROM IBSADATE) || '월-' 
       || EXTRACT(DAY FROM IBSADATE) || '일' || TO_CHAR(IBSADATE, 'DAY') "입사일" 
FROM TBL_INSA
WHERE SUBSTR(SSN, 8, 1) = '1';
--==>>
/*
홍길동	771212-1022432	1998년-10월-11일일요일
이순신	801007-1544236	2000년-11월-29일수요일
김정훈	790304-1788896	2000년-10월-1일일요일
한석봉	811112-1566789	2004년-8월-13일금요일
장인철	780506-1625148	1998년-3월-16일월요일
나윤균	810810-1552147	2003년-10월-10일금요일
김종서	751010-1122233	1997년-8월-8일금요일
정한국	760909-1333333	1999년-10월-16일토요일
이상헌	781010-1666678	2001년-11월-29일목요일
엄용수	820507-1452365	2000년-8월-28일월요일
이성길	801028-1849534	2004년-8월-8일일요일
박문수	780710-1985632	1999년-12월-10일금요일
홍길남	801010-1111111	2001년-9월-7일금요일
김인수	731211-1214576	1995년-2월-23일목요일
우재옥	801103-1654442	2000년-10월-1일일요일
김영길	801216-1898752	2000년-10월-18일수요일
이남신	810101-1010101	2001년-9월-7일금요일
지재환	771115-1687988	2001년-1월-21일일요일
이정석	820505-1325468	2005년-9월-26일월요일
최석규	770129-1456987	1998년-10월-15일목요일
박세열	790509-1635214	2000년-9월-10일일요일
문길수	721217-1951357	2001년-12월-10일월요일
지수환	820305-1475286	2004년-1월-21일수요일
홍원신	690906-1985214	2003년-3월-16일일요일
허경운	760105-1458752	1999년-5월-4일화요일
산마루	780505-1234567	2001년-7월-15일일요일
이기상	790604-1415141	2001년-6월-7일목요일
김싱식	800715-1313131	1999년-12월-12일일요일
정상호	810705-1212141	1999년-10월-16일토요일
전용재	800605-1456987	2004년-8월-13일금요일
김신제	800709-1321456	2003년-8월-8일금요일
...
SELECT BUSEO "부서", JIKWI "직위", COUNT(JIKWI) "인원수", SUM(BASICPAY+SUDANG) "급여합"
...
SELECT BUSEO "부서", JIKWI "직위", COUNT(JIKWI) "인원수"
FROM TBL_INSA
GROUP BY BUSEO, JIKWI
HAVING COUNT(JIKWI) >=5;
--==>>
/*
영업부	사원	8
영업부	대리	6
...
FROM
(
    SELECT BUSEO "부서", COUNT(*) "부서별남자인원수"
    FROM TBL_INSA
    WHERE SUBSTR(SSN, 8, 1) = '1'
    GROUP BY BUSEO
)T
...
FROM
(
    SELECT NUM "번호", NAME "이름", SSN "주민번호", IBSADATE "입사일", CITY "출신도", TEL "전화번호"
         , BUSEO "부서", JIKWI "직위", BASICPAY "기본급", SUDANG "수당"
...
1062	    박민지	960907-2234567	21-04-09	    서울  	010-6666-6666	기획부	대리	5000000	500000	1
1046    	허경운	760105-1458752	99-05-04    	경남  	017-3333-3333	총무부	부장	2650000	150000	3
1001    	홍길동	771212-1022432	98-10-11	    서울  	011-2356-4528	기획부	부장	2610000	200000	4
1003	    이순애	770922-2312547	99-02-25	    인천  	010-4231-1236	개발부	부장	2550000	160000	5
*/
...
FROM TBL_INSA
WHERE BASICPAY IN (SELECT MAX(BASICPAY) "기본급"
                   FROM TBL_INSA
                   GROUP BY BUSEO);
--==>>
/*
...
(
    SELECT NAME "이름", CITY "지역", BASICPAY+SUDANG "급여"
        , RANK() OVER(PARTITION BY CITY ORDER BY BASICPAY+SUDANG) "순위"
    FROM TBL_INSA
)T
WHERE T.순위 =1;
--==>>
...
(
    SELECT CITY "지역", AVG(BASICPAY+SUDANG) "평균급여"
         , RANK() OVER(ORDER BY AVG(BASICPAY+SUDANG)) "순위"
    FROM TBL_INSA
    GROUP BY CITY
)T
WHERE T.순위 =1;
...
한혜림	영업부	서울	    5000000	500000	5500000	5010000	490000
박민지	기획부	서울	    5000000	500000	5500000	5010000	490000
*/
...
SELECT T.부서, T.평균급여
     , CASE WHEN T.평균급여>2000000 THEN 'A등급'
            ...
            WHEN T.평균급여<1500000 THEN 'C등급'
            ELSE '등급없음'
       END "등급"
FROM
(
    SELECT BUSEO "부서", TRUNC(AVG(BASICPAY+SUDANG)) "평균급여"
    FROM TBL_INSA
    GROUP BY BUSEO
)T;
--==>
